function plot_data(in_data,out_data,settings,holdon)
...
if strcmp(settings.gnss_outage,'on')
    h(4)=plot(in_data.GNSS.pos_ned(2,200),in_data.GNSS.pos_ned(1,199),'rs', 'MarkerSize',9, 'MarkerFaceColor',[1,0,0]);
end
title('Trajectory')
ylabel('North [m]')
xlabel('East [m]')
if strcmp(settings.gnss_outage,'on')
    legend(h,'GNSS position estimate','GNSS aided INS trajectory','Start point','GNSS Outage start')
else
