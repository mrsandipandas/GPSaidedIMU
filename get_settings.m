...
% Standard deviations, need to be squared
% settings.sigma_acc = 4; % [m/s^2]
% settings.sigma_gyro = 0.025*pi/180; % [rad/s]
% settings.sigma_acc_bias = 0.1; % [m/s^2.5]
% settings.sigma_gyro_bias = 0.001*pi/180; % [rad/s^1.5]

% GNSS position measurement noise covariance (R)
% Standard deviations, need to be squared
settings.sigma_gps = 3/sqrt(3); %[m] 
settings.sigma_speed = 30; %[m/s]  Trim here
...
% For GPS outage without 
% settings.factp(1) = 10;                                 % Position [m]
% settings.factp(2) = 5;                                  % Velocity [m/s]
% settings.factp(3:5) = (pi/180*[1 1 20]');               % Attitude (roll,pitch,yaw) [rad]
% settings.factp(6) = 0.005;                               % Accelerometer biases [m/s^2]
% settings.factp(7) = (0.01*pi/180);                      % Gyro biases [rad/s]                               
