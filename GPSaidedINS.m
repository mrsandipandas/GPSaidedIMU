...
for k=2:N
    ...
    % Equation 23 in Sensor Fusion GPS+IMU
    y = [in_data.GNSS.pos_ned(:,ctr_gnss_data);
         in_data.SPEEDOMETER.speed(:,ctr_gnss_data);
         zeros(2,1)];
    
    
    % H = [eye(3) zeros(3,12)];
    Rn2p = get_Rb2p()*q2dcm(x_h(7:10))';
    % Equation 22 in Sensor Fusion GPS+IMU
    H = [eye(3)    zeros(3,3) zeros(3,9);
         zeros(3,3)   Rn2p    zeros(3,9)];
        %zeros(2,3)  A*Rn2p     zeros(2,9)];
    
    % R = settings.sigma_gps^2*eye(3);
    % Equation 21 for the measurement noise in Sensor Fusion GPS+IMU
    R = [settings.sigma_gps^2*eye(3)  zeros(3,3);
         zeros(1,3)    settings.sigma_speed^2*eye(1) zeros(1,2);
         zeros(2,4)    settings.sigma_non_holonomic^2*eye(2)];
    ...
    if t(k)==in_data.GNSS.t(ctr_gnss_data)
        if t(k)<settings.outagestart || t(k)>settings.outagestop || ~strcmp(settings.gnss_outage,'on')
            ind(1:3) = 1;
            disp(['Runs the GNSS-aided INS ', num2str(t(k))])
        else
            disp(['Runs the GNSS-non-aided INS ', num2str(t(k))])
        end
